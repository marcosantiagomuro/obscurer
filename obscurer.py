...
def getoui():
    ...
    try:
        urllib.request.urlretrieve(
            "https://linuxnet.ca/ieee/oui.txt", filename="oui.txt")
        return 0
    except Exception:
...
def generate_mac():
    mac_addresses = []
    # Check if the oui.txt file exists in the same directory as the script.
    ...
    mac_addresses = []
    for i in ouiarray:
        # Generate a MAC address and add to the array.
        mac_addresses.append(
            i + ":{0}:{1}:{2}".format(rand_hex(), rand_hex(), rand_hex()))
